...
(async () => {
  ...
  try {
    ...
    const [boot, stepped, afterInput] = result;
    assert(boot.phase === 'PLAYING', 'expected PLAYING phase at boot');
    assert(stepped.player.y >= boot.player.y, 'expected gravity/physics to advance after stepping time');
    ...
    fs.writeFileSync(path.join(outputDir, 'systemic-loop-smoke.json'), JSON.stringify({ boot, stepped, afterInput }, null, 2));
